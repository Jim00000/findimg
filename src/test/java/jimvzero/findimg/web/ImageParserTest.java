...
public class ImageParserTest {
	...
	private final static Logger log = LogManager.getLogger(ImageParserTest.class);
	...
	void testcase1() {
		try {
			String filepath = ImageParserTest.class.getClassLoader().getResource("testcases/parse-case-1.html")
					.getFile();
	...
	void testcase2() {
		try {
			String filepath = ImageParserTest.class.getClassLoader().getResource("testcases/parse-case-2.html")
					.getFile();
			final String html = FileUtils.readFileToString(new File(filepath), Charset.defaultCharset());
			parser.parse(html);
			List<URL> urls = parser.getImage("https?:\\/\\/i\\.imgur\\.com\\/[0-9a-zA-Z]*\\.(jpg|jpeg|png)");
	...
	void testcase3() {
		try {
			String filepath = ImageParserTest.class.getClassLoader().getResource("testcases/parse-case-3.html")
					.getFile();
			final String html = FileUtils.readFileToString(new File(filepath), Charset.defaultCharset());
			parser.parse(html);
			List<URL> urls = parser
					.getImage("https?://truth\\.bahamut\\.com\\.tw/[0-9a-zA-Z/]*\\.([jJ][pP][gG]|[pP][nN][gG])");
